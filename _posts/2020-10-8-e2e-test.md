...
过P0用例用时比开发用时还要久。尤其是维护长期迭代的业务，开发不太可能在每次修改之后都去检查所有的用例，长此以往，不常用的场景可能就会有bug。
...
describe(caseName, function(){
    ...
    before(function(){
        ...
        try {
            self.driver = driver.session(sessionConfig).windowSize(1024, 2880).config({
                pageloadTimeout: 10000, // page onload timeout
